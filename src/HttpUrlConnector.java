...
public class HttpUrlConnector {
	private static final String RESERVE_PARAMS_FILENAME = "/usr/share/nginx/www/wiki/data/pages/pknu_studyroom_auto_reserve/reserve_params.txt";
	...
	private static final String ALREADY_FULL = "alert (\"�����Ͻ� �ð��� �ٸ� ������ �����մϴ�.\");"; // already full result code
	private static final String YOU_HAVE_ANOTHER_ROOM = "alert (\"�Ϸ翡 3�ð� �̻� �����Ͻ� �� �����ϴ�.\");";
	private static final String RESERVED_STRING = "opener.alert(\"����ó�� �Ǿ����ϴ�.\");";
	...
	{		
			...
			while(!isVisited[i]) {
				isVisited[i] = true;
				switch(states[i]=sendReservePostRequest(roomID[i],reserveParameters)){
				case 1:{
					Print("ROOM ID - " + roomID[i] + " IS FULL");
					break;
				}
				case 2:{
					Print("ROOM ID - " + roomID[i] + " IS NOT RESERVED YOU HAVE ANOTHER ROOM TODAY!");
					break;
				}
				case 3:{
		...
		try {
			...
			while((line=bf.readLine())!=null)
			{
				if(line.trim().startsWith("alert")){
					Print(line.trim()+" (len: "+line.trim().length()+")");
					if(line.trim().equals(ALREADY_FULL))
					//if(line.trim().length() == 45)
					{
						state = 1;
					//}else if(line.trim().length() == 43)
					}else if(line.trim().equals(YOU_HAVE_ANOTHER_ROOM))
					{
						state = 2;
					}
				}else if(line.trim().startsWith("opener.alert")){
					if(line.trim().equals(RESERVED_STRING))
					//if(line.trim().length() == 34)
					{
						Print(line.trim()+" (len: "+line.trim().length()+")");
						state = 3;
					}
